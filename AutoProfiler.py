...
class AutoProfiler:
    def __init__(self,powerzones=None, command=None,args=None, dt=0.1):
        self.dt=dt
        #Timestep in nS
        self.dtns=dt*1e9
        if (powerzones is not None) and (path is not None): 
    ...
    def start(self):
        ...
        process=Popen(['/usr/bin/sleep', '4'], stdout=PIPE, stderr=PIPE)
    ...
    def __repr__(self):
        s=f"Profile time: {99999999999} s, N* samples {9999999}, dt {self.dt} s\nZones:\n"
        for zone in self.powerzones:
            s+=f"\t{zone.name}\n"
        return s   
    ...
    def plot_profile(self,filename="default.hdf5"):
        self.profiles=dict()
        plt.figure()
        with h5py.File(filename, 'r') as f:
            for zone_group in f.keys():
                for zone_dataset in f[zone_group].keys():
                    print(f[zone_group][zone_dataset])         
                    plt.plot(f[zone_group][zone_dataset][:],label=f"{zone_group}/{zone_dataset}")
            plt.legend()
    '''
    def plot(self):
